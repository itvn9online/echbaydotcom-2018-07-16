...
foreach ( $arr as $k => $v ) {
	// nếu có số lượng
	if ( isset( $_POST['t_soluong'][$k] )  > 0 ) {
		$strFilter .= "," . $v;
		
		$arr_shop_cart[$v] = (int) $_POST['t_soluong'][$k];
		$arr_shop_cart_size[$v] = isset( $_POST['t_size'][$k] ) ? $_POST['t_size'][$k] : '';
	}
}
//echo $strFilter . "\n";
...
//	echo date_time - $lan_gui_don_truoc . "\n";
	...
	if ( date_time - $lan_gui_don_truoc < 300 ) {
		ket_thuc_qua_trinh_dat_hang( $strsql->ID, $strsql->post_title, 'Cảm ơn bạn, chúng tôi đang tiếp nhận và xử lý đơn hàng của bạn.' );
	}
}
...
$arr = array(
	'__eb_hd_ten' => $t_ten,
	'__eb_hd_diachi' => $t_diachi,
	'__eb_hd_ghichu' => $t_ghichu,
	'__eb_hd_dienthoai' => $t_dienthoai,
//	'__eb_hd_trangthai' => $hd_trangthai,
	
//	'__eb_hd_customer_info' => $_POST ['hd_customer_info'],
	/*
	'__eb_hd_url' => $_POST ['hd_url'],
	'__eb_hd_title' => $_POST ['hd_title'],
	'__eb_hd_timezone' => $_POST ['hd_timezone'],
	'__eb_hd_lang' => $_POST ['hd_lang'],
	'__eb_hd_usertime' => $_POST ['hd_usertime'],
	'__eb_hd_window' => $_POST ['hd_window'],
	'__eb_hd_document' => $_POST ['hd_document'],
	'__eb_hd_screen' => $_POST ['hd_screen'],
	'__eb_hd_agent' => $_POST ['hd_agent'],
	*/
	'__eb_hd_ip' => $client_ip,
);
//print_r( $arr );

$str_for_custom_order = '';
foreach ( $arr as $k => $v ) {
	$str_for_custom_order .= ',"' . $k . '" : "' . str_replace( '"', '&quot;', $v ) . '"';
}
$str_for_custom_order = substr( $str_for_custom_order, 1 );
...
foreach ( $arr as $v ) {
	...
	if ( $str_for_custom_order == '' ) {
		$str_for_custom_order .= $v;
	} else {
		$str_for_custom_order .= '<br>' . $v;
	}
}
...
$page = array(
	...
	'post_title' => $hd_mahoadon,
	'post_excerpt' => '[' . $product_js_list . ',{' . $str_for_custom_order . '}]',
	'post_status' => 'private',
	'post_type' => 'shop_order',
	'post_parent' => 0,
	'post_name' => $hd_mahoadon,
);
//print_r( $page );
