...
//		print_r( $post );
		...
		$post->p_link = $this->_p_link( $post->ID );
//		$post->p_link = $post->guid;
		$post->trv_tieude = $post->post_title;
		$post->trv_id = $post->ID;
		$post->trv_masanpham = $this->get_post_meta( $post->ID, '_eb_product_sku', true, $post->ID );
		$post->trv_gioithieu = $post->post_excerpt;
		$post->ngaycapnhat = date( 'd/m/Y', strtotime( $post->post_modified ) );
		
		$post->trv_mua = (int) $this->get_post_meta( $post->ID, '_eb_product_buyer', true, 0 );
		$post->trv_giaban = number_format( (int) $this->get_post_meta( $post->ID, '_eb_product_oldprice', true, 0 ) );
		$post->trv_giamoi = number_format( (int) $this->get_post_meta( $post->ID, '_eb_product_price', true, 0 ) );
		...
		$post->ant_ten = isset ($ant[0]->name ) ? '<a href="' . $this->_c_link( $ant[0]->term_id ) . '">' . $ant[0]->name . '</a>' : '';
		...
		$post->trv_img = $this->get_post_img( $post->ID );
...
//		print_r( $ant );
		...
		//
		$row = array(
			'p_link' => $this->_p_link( $post->ID ),
			
//			'trv_tieude' => get_the_title(),
			'trv_tieude' => $post->post_title,
			'trv_id' => $post->ID,
			'trv_masanpham' => $this->get_post_meta( $post->ID, '_eb_product_sku', true, $post->ID ),
			'trv_gioithieu' => $post->post_excerpt,
			'ngaycapnhat' => date( 'd/m/Y', strtotime( $post->post_modified ) ),
			
			'trv_img' => $trv_img,
			'trv_mua' => (int) $this->get_post_meta( $post->ID, '_eb_product_buyer', true, 0 ),
			'trv_giaban' => number_format( (int) $this->get_post_meta( $post->ID, '_eb_product_oldprice', true, 0 ) ),
			'trv_giamoi' => number_format( (int) $this->get_post_meta( $post->ID, '_eb_product_price', true, 0 ) ),
			'trv_color_count' => 1,
			...
			'ant_ten' => isset ($ant[0]->name ) ? '<a href="' . $this->_c_link( $ant[0]->term_id ) . '">' . $ant[0]->name . '</a>' : '',
		);
...
//		print_r($row);
	...
	public function str_template( $f, $arr = array(), $dir = EB_THEME_HTML ) {
		...
		if (file_exists($f)) {
			...
			//
			return $this->html_template( $f, $arr );
		}
...
//			$page_name = 'test-page';
		...
		$sql = $this->q("SELECT ID, post_title, post_excerpt
		FROM
			" . $wpdb->posts . "
		WHERE
...
//			echo $f . '<br>';
	...
	*/
	function add_css ( $arr = array(), $include_now = 0 ) {
		$this->add_css_js_file( $arr, '.css', $include_now );
	}
	...
	function add_js ( $arr = array(), $include_now = 0 ) {
		$this->add_css_js_file( $arr, '.js', $include_now );
	}
	...
	function add_full_js ( $arr = array(), $type_add = 'import' ) {
		if ( $type_add == 'import' ) {
			foreach ( $arr as $v ) {
				// v2
				echo $this->import_js( $v . '?v=' . web_version );
			}
		}
		else {
			foreach ( $arr as $v ) {
	...
	function _last ( $arr ) {
		return $this->_end($arr);
	}
	function _begin ( $arr ) {
		return $arr[0];
	}
	function _first ( $arr ) {
		return $this->_begin($arr);
	}
...
//		print_r($arr);
		foreach ( $arr as $v ) {
			$strCacheFilter .= $this->_end ( explode( '/', $v ) );
		}
		$strCacheFilter = preg_replace ( '/[^a-zA-Z0-9]+/', '-', $strCacheFilter );
//		echo 'Cache: ' . $strCacheFilter . '<br>'; exit();
		
		//
		$f_content = $this->get_static_html ( $strCacheFilter, '', '', $check_time );
		if ($f_content == false) {
			$f_content = '';
			...
			foreach ( $arr as $v ) {
//				$f_filename .= $v;
				$f_filename .= $this->_end( explode( '/', $v ) );
...
//				echo gettype( $new_content ) . '<br>';
				...
				$this->create_file ( $f_save, $new_content );
				
				//
				$this->log_admin( 'Create static file: ' . $strCacheFilter );
			}
//			exit();
...
//				echo 'URL: ' . $f_url . '<br>';
				...
				*/
				...
				$f_content = $this->import_js( $f_url );
	...
	//				$c = '<link rel="stylesheet" href="' . $f_url . '?v=' . web_version . '" type="text/css" />';
			...
			$this->get_static_html ( $strCacheFilter, $f_content, '', $check_time );
		}
	...
	function import_js ( $js ) {
		/* */
		return '<script type="text/javascript" async src="' . $js . '"></script>';
		/* */
...
//		global $wpdb;
		...
		if ( $in_cache == 0 ) {
			...
			$a = $this->get_static_html ( $strCacheFilter );
		}
...
//			$a = str_replace( '/category/', '/', $a );
			...
			/*
			$this->get_static_html ( $strCacheFilter, $a );
		}
		*/
			...
			*/
			...
			if ( $a == '' ) {
				if ( $slug == id_default_for_get_sidebar ) {
					...
					$a = $this->echbay_sidebar( id_default_for_get_sidebar, $css, $div, $in_cache );
				}
			} else {
				$a = str_replace( '/./', '/', $a );
...
//			$_file = '../' .$_file;
		...
		$str = $this->del_line( $str );
//		echo $str . '<br />';
		
		// lưu log
		error_log( date( '[d-m-Y H:i:s]', date_time ). '; IP: ' .$this->i(). '. MYSQL Warning: ' .$str. " --------------------> URL: " .$this->full_url(). "\n", 3, $_file );
...
//		$this->eb_postmeta( eb_config_id_postmeta, $key, $val );
...
//		print_r( $__cf_row );
	...
	public function log_click($m) {
		...
		$this->eb_postmeta( eb_log_click_id_postmeta, '__eb_log_click', $m );
	}
	...
	public function log_user($m) {
		...
		$this->eb_postmeta( eb_log_user_id_postmeta, '__eb_log_user', $m );
	}
...
//		echo $m . "\n";
		...
		$this->eb_postmeta( eb_log_user_id_postmeta, '__eb_log_admin', $m );
	}
	...
	public function log_admin_order($m, $order_id) {
		...
		$this->eb_postmeta( eb_log_user_id_postmeta, '__eb_log_invoice' . $order_id, $m );
	}
	...
	public function log_search($m) {
		...
		$this->eb_postmeta( eb_log_search_id_postmeta, '__eb_log_search', $m );
	}
...
//		$headers[] = 'Date: ' . gmdate('d M Y H:i:s Z', NOW);
	...
	public function lnk_block_email ($em) {
		return web_link . 'block_email&e=' .base64_encode( $em ). '&c=' . $this->mdnam( $em );
	}
	...
	public function send_email($to_email, $title, $message, $headers = '', $bcc_email = '', $add_domain = 1) {
		...
		if ($__cf_row ['cf_sys_email'] == 1) {
			return $this->send_mail_phpmailer ( $to_email, '', $title, $message, '', $bcc_email );
		}
		...
		if ($headers == '') {
			...
			if ( $__cf_row['cf_email_note'] != '' ) {
				$headers = $this->build_mail_header ( $__cf_row['cf_email_note'] );
			} else if ( $__cf_row['cf_email'] != '' ) {
				$headers = $this->build_mail_header ( $__cf_row['cf_email'] );
			} else {
				*/
				$headers = $this->build_mail_header ( 'noreply@' . $chost );
				/*
			}
			*/
		}
		...
		//
		$message = $this->str_template ( 'html/mail/mail.html', array (
				'tmp.message' => $message,
		...
		*/
		...
		if ($bcc_email != '') {
			...
			foreach ( $bcc_email as $v ) {
				...
				if ( $v != '' && $this->check_email_type( $v ) == 1 ) {
					$headers .= "\r\n" . 'BCC: ' . $v;
				}
			}
		}
...
//		$mail = new PHPMailer ();
		...
		if ($to == '' || $this->check_email_type ( $to ) == false) {
			return false;
		}
		if ($to_name == '') {
			$to_name = explode ( '@', $to );
		...
		*/
		...
		if ( $a != 1 ) {
			...
			$this->send_email($to, $subject, $message, '', $bcc_email, 0 );
	...
	*/
	public function load_ads_v2 ( $type = 0, $posts_per_page = 20, $_eb_query = array(), $op = array() ) {
		...
		return $this->load_ads( $type, $posts_per_page, $op['data_size'], $_eb_query, $op['offset'], $op['html'] );
	}
...
//				$___eb_ads__not_in .= ',' . $post->ID;
				...
				//
				$p_link = $this->get_post_meta( $post->ID, '_eb_ads_url', true );
				if ( $p_link == '' ) {
					$p_link = 'javascript:;';
				}
	//			echo $p_link . '<br>';
				...
				// lấy ảnh từ bài viết
				$trv_img = $this->get_post_img( $post->ID );
...
//					print_r( $trv_img );
				...
				// lấy ảnh từ meta box (với một số loại bài viết)
					...
					$trv_img = $this->get_post_meta( $post->ID, '_eb_product_avatar', true );
				}
				*/
				
				//
				$youtube_id = $this->get_youtube_id( $this->get_post_meta( $post->ID, '_eb_ads_video_url', true ) );
				$youtube_url = '';
				$youtube_avt = '';
				if ( $youtube_id != '' ) {
...
//			header ( 'Location:' . $url, true, 301 );
	...
	public function _p_link ($id, $seo = '') {
		...
		$a = $this->get_static_html ( $strCacheFilter, '', '', eb_default_cache_time );
		if ($a == false) {
			...
			//
			$this->get_static_html ( $strCacheFilter, $a, '', 60 );
		}
//		echo $a . '<br>' . "\n";
	...
	public function _c_link ( $id, $taxx = 'category' ) {
		...
		$a = $this->get_static_html ( $strCacheFilter, '', '', eb_default_cache_time );
//		$a = $this->get_static_html ( $strCacheFilter, '', '', 5 );
		if ($a == false) {
...
//		echo $a . '<br>' . "\n";
	...
	public function _b_link($id, $seo = '') {
		return $this->_p_link($id);
	}
	// blog group
	public function _bs_link($id, $seo = '') {
		return $this->_c_link( $id );
	}
...
//			print_r( $parent_cat );
			...
			//
			$group_go_to = ' <li><a href="' . $this->_c_link($parent_cat->term_id) . '">' . $parent_cat->name . '</a></li>' . $group_go_to;
			
			// tìm tiếp nhóm cha khác nếu có
			if ( $parent_cat->parent > 0 ) {
				$return_id = $this->create_html_breadcrumb( $parent_cat );
			}
		}
...
//				print_r( $a );
				...
				$a = $this->get_post_meta( $id, '_eb_product_avatar', true );
			}
...
//			print_r( $a );
			$a = ! empty( $a[0] ) ? esc_url( $a[0] ) : $this->get_post_meta( $id, '_eb_product_avatar', true );
			*/
...
//			print_r($v);
			...
			//
//			$c_link = $this->_c_link( $v->term_id, $web_link . '?cat=' . $v->term_id );
			$c_link = $web_link . '?cat=' . $v->term_id;
			
			//
			$cat_order = 0;
			if ( $this_id == 0 ) {
				$cat_order = (int) $this->get_post_meta( $v->term_id, '_eb_category_order', true, 0 );
			}
			
			//
			$str .= ',{id:' . $v->term_id . ',ten:"' . $this->str_block_fix_content ( $v->name ) . '",lnk:"' . $c_link . '",order:' . $cat_order . ',arr:[' . $this->get_full_category_v2 ( $v->term_id, $taxx ) . ']}';
		}
		$str = substr ( $str, 1 );
